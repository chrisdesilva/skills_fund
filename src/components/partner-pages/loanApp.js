...
const LoanApp = (props) => {
	...
	const redirectLoanApp = () => {
		window.open(
			`https://sf.privateloan.studentloan.org/external/LoanApplication.do?lenderCode=${segment}`,
			'_blank',
			'noopener noreferrer'
		);
	};
	...
	return (
		<div className={props.modal ? 'loanApp opacity' : 'loanApp'}>
			...
			<div className="loanApp__content">
				<h2>
					{props.schoolname} Tuition {props.costOfLiving ? 'and Cost of Living' : null} Financing
				</h2>
				<div className="loanApp__img">
					<Img fluid={props.skfLogo} alt="Skills Fund logo" />
				</div>
				...
				<form onSubmit={handleSubmit} className={`loanApp__form  ${props.schoolname}_apply_now program-apply`}>
					...
					<select onChange={updateLoanUrl}>
						{props.loanInfo.map((school, i) => (
							<option key={school.name} value={school.segment}>
								{school.name}
							</option>
						))}
					</select>
					{submitted ? (
						<span>
							...
							<a href="mailto:tech@skills.fund">Tech@Skills.Fund</a>.
						</span>
					) : (
						<input className="btn btn--submit" type="submit" value="APPLY NOW" />
					)}
				</form>
			</div>

			<div className="loanApp__subContent">
				<p onClick={() => toggleDisclaimers(!disclaimers)}>
					<span className="disclaimers">Disclaimers</span>
				</p>
